...
export default function Game() {
  ...
  // Escape closes whatever is up, the way a phone's back gesture would. The
  // verdict dialog is a native one and handles its own Escape, so this only
  // ever reaches the bottom sheets.
  useEffect(() => {
  ...
  // Any sheet that happened to be up goes away with it: the verdict opens in
  // the top layer, and a sheet left underneath would only be in the way of
  ...
  return (
    <div className="room">
      ...
      {/* A native <dialog>, so it opens in the top layer -- above the bottom
          sheets as well as the table, which is the right order for the one
      ...
      <section className="controls">
        {game.over ? (
          ...
          <>
            <button className="commit prime" onClick={() => start(seats)}>
              Deal again
            </button>
            <span className="hint">
